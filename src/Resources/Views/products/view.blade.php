...
<x-shop::layouts>
            ...
            >
                ...
                >
                    ...
                    <div class="container px-[60px] max-1180:px-0">
                        ...
                        @php
                        // For configurable products, check the variants
                        ...
                        if ($product->type == 'configurable') {
                            foreach ($product->variants as $variant) {
                                if ($variant->special_price_from && $variant->special_price_to) {
                                    $hasSpecialPrice = true;
                                    $specialPriceFrom = $variant->special_price_from;
                                    $specialPriceTo = $variant->special_price_to;
                                    break;
                                }
                            }
                        } else {
                            $hasSpecialPrice = $product->special_price_from && $product->special_price_to;
                        ...
                        @php
                            ...
                            $start = \Carbon\Carbon::parse($specialPriceFrom);
                            $end = \Carbon\Carbon::parse($specialPriceTo);
                        @endphp

                        @if ($now->between($start, $end))
                            <div style="background-color:black;" class="mt-4 border p-2.5 rounded-lg shadow-md text-center">
                                <p class="text-2xl font-semibold mb-1 text-white">⚡ Flash Sale Ends In:</p>
                                <p class="text-sm text-gray-400 mb-2">Limited-time offer! Grab it now!</p>
                                <div class="flex items-center justify-center space-x-2 text-white text-2xl">
                                    ⏳ &nbsp;<span id="countdown-timer-{{ $product->id }}"></span>
                                </div>
                            </div>

                            @push('scripts')
                                <script>
                                    function updateCountdown{{ $product->id }}() {
                                        const endTime = new Date("{{ $end->toIso8601String() }}").getTime();
                                        const now = new Date().getTime();
                                        const timeLeft = endTime - now;

                                        const timerElement = document.getElementById('countdown-timer-{{ $product->id }}');
                                    ...
                                    const interval{{ $product->id }} = setInterval(updateCountdown{{ $product->id }}, 1000);
                                    updateCountdown{{ $product->id }}(); // Initial call
                                </script>
    @endpush
    @endif
    @endif
    ...
    <div class="mt-8 flex max-w-[470px] gap-4 max-sm:mt-4">
        ...
        @if ($product->getTypeInstance()->showQuantityBox())
            <x-shop::quantity-changer name="quantity" value="1"
        ...
        @if (core()->getConfigData('sales.checkout.shopping_cart.cart_page'))
            ...
            <x-shop::button type="submit"
                class="secondary-button w-full max-w-full max-md:py-3 max-sm:rounded-lg max-sm:py-1.5"
                button-type="secondary-button" :loading="false" :title="trans('shop::app.products.view.add-to-cart')" :disabled="!$product->isSaleable(1)"
                ::loading="isStoring.addToCart" ::disabled="isStoring.addToCart" @click="is_buy_now=0;" />
    ...
    @if (core()->getConfigData('sales.checkout.shopping_cart.cart_page'))
        ...
        @if (core()->getConfigData('catalog.products.storefront.buy_now_button_display'))
            <x-shop::button type="submit"
                ...
                button-type="primary-button" :title="trans('shop::app.products.view.buy-now')" :disabled="!$product->isSaleable(1)" ::loading="isStoring.buyNow"
                @click="is_buy_now=1;" ::disabled="isStoring.buyNow" />
        @endif
    ...
    <div class="mt-10 flex gap-9 max-md:mt-4 max-md:flex-wrap max-sm:justify-center max-sm:gap-3">
        ...
        <div class="flex cursor-pointer items-center justify-center gap-2.5 max-sm:gap-1.5 max-sm:text-base"
            role="button" tabindex="0" @click="is_buy_now=0; addToCompare({{ $product->id }})">
